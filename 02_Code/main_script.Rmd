...
###### NOTE #######
# This code was written to run a lasso algorithm to see, which variables predict the good/bad classifier best.
# Since the solution was to omit non-youtube videos and ted-ed videos, this code has lost its purpose 
...
# SentiWords with other dimensions

# Analyse outcome variables for composite outcome

```{r}
outcomes = data %>% select(views_youtube, likes, dislike, no_comments) %>% na.omit()

cor(outcomes)
corrplot::corrplot(cor(outcomes), method = "number")


#Dislikes and number of comments
data %>% ggplot(aes(x=log(dislike), y= log(no_comments))) + geom_point()

#Views and likes
data %>% ggplot(aes(x=log(views_youtube), y= log(likes))) + geom_point()


#likes and dislikes
data %>% ggplot(aes(x=log(likes), y= log(dislike))) + geom_point()

#views and number comments
data %>% ggplot(aes(x=log(likes), y= log(no_comments))) + geom_point()

#composite outoome variable
data = data %>% mutate(
  z_views_youtube = scale(views_youtube),
  z_likes = scale(likes),
  z_dislike = scale(dislike),
  z_no_comment = scale(no_comments)
)
outcomes
z_outcomes = scale(outcomes)
data = data %>% mutate(
  engagement = (z_views_youtube + 2*z_likes + 3*z_dislike + 4*z_no_comment)/10
)
...
# library(psych)
# if (Daten=="Rohdaten") {
# data2 <- as.matrix(na.omit(data[, Variablen, drop=FALSE]))
# } else {
# data <- as.matrix(data)
# N <- as.numeric(data[1,1])  # Stichprobengroesse einlesen
# matr <- data[4:nrow(data),] # Dreiecksmatrix einlesen
# # Dreiecksmatrix in symmetrische Matrix verwandeln
# matr[upper.tri(matr)] <- t(matr)[upper.tri(matr)]
# SD <- as.matrix(data[3,])   # Standardabweichungen einlesen
# matr <- SD%*%t(SD)*matr # Kovarianzmatrix berechnen
# data2 <- matr
# }
...
kmo <- KMO(outcomes); n.obs <- NA


# Hauptkomponenten- oder Hauptachsenanalyse

m <- principal(outcomes, nfactors=2, rotate="none", covar=FALSE, n.obs=n.obs)
Faktorwerte <- m$scores


# if (Rotation=="oblimin") list("Anti-image correlation"=kmo$Image, KMO=kmo, Analyse=m, Strukturmatrix=m$Structure) 

list("Anti-image correlation"=kmo$Image, KMO=kmo, Analyse=m)
...
# affective frequency using log(dislike)
af_freq_dislike <- ggplot(talks, aes(affect, log(dislike))) +
  geom_point() +labs(title= "Log(dislike) by % of affect words") + tilt_theme +
  labs(x = "% of affect words in a talk", y = "log(dislike)")+ geom_smooth(method = "lm")
af_freq_dislike



# average sentiment using log(views_youtube)
ave_sent_views <- ggplot(talks, aes(mean_intensity, log(views_youtube))) +
  geom_point() +labs(title= "Log(views_youtube) by mean_intensity") + tilt_theme +
  labs(x = "mean_intensity", y = "log(views_youtube)")+ geom_smooth(method = "lm")
ave_sent_views 

# average sentiment using log(dislike)
ave_sent_dislike <- ggplot(talks, aes(affect, log(dislike))) +
  geom_point() +labs(title= "Log(dislike) by mean_intensity") + tilt_theme +
  labs(x = "mean_intensity", y = "log(dislike)")+ geom_smooth(method = "lm")
ave_sent_dislike
...
talks <-   cbind(talks, pred_m_comments = prediction)
