...
```{r}
outcomes = data %>% select(views_youtube, views, likes, dislike, like_dislike_ratio, no_comments) %>% na.omit()
...
# poisson regression
poiss_aff <- glm(views_youtube ~ affect + mean_intensity + abs_mean_intensity + poly(days_since_upload, 2), family = "poisson", data = talks)
summary(poiss_aff)
...
# plot the points (actual observations), regression line
plot_affect2 <- ggplot(talks, aes(mean_intensity,log(views_youtube))) + 
  geom_point() +
  geom_smooth(aes(mean_intensity, pred_affect2)) +
  labs(title= "Log(views_youtube) by % of affect words", x = "% of affect words in a talk", y = "log(views_youtube)")
