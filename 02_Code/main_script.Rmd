...
for (i in 1:length(unique(tags_tbl$id))){
...
# w <- abs(rnorm(nrow(edge_list)))

# adj_matrix = edgelist_to_adjmat(edge_list[,1:2], undirected = TRUE, multiple = TRUE)

# adj = get.adjacency(graph, sparse = FALSE)

# adj = as_adjacency_matrix(graph, sparse=FALSE, edges = TRUE)
...
# graph_list = as.matrix(edge_list)

graph = graph_from_adjacency_matrix(adj, mode = "undirected", weighted = TRUE)
...
```{r}
main_tags = c("environment","society","science","entertainment","technology")
...
tags = tags %>% group_by(id) %>% mutate(
  ...
  community_chr = case_when(
    ...
    main_tag == 2 ~ "society",
    main_tag == 4 ~ "entertainment",
    main_tag == 3 ~ "science",
    main_tag == 5 ~ "technology"
  )
)
...
data = data %>% mutate(
  community_chr = case_when(
    ...
    community == 2 ~ "society",
    community == 4 ~ "entertainment",
    community == 3 ~ "science",
    community == 5 ~ "technology"
  )
)
...
cor_table = talks %>% select(affect, posemo, negemo, mean_intensity, abs_mean_intensity) %>% cor()
...
# poisson regression
poiss_aff <- glm(views_youtube ~ affect + mean_intensity + abs_mean_intensity + days_since_upload, family = "poisson", data = talks)
summary(poiss_aff)
...
# add predicted log(views_youtube) to talks
prediction = predict(poiss_aff)
talks <-   cbind(talks, pred_affect2 = prediction)

# plot the points (actual observations), regression line
plot_affect2 <- ggplot(talks, aes(affect,log(views_youtube))) + 
  geom_point() +
  geom_smooth(aes(affect, pred_affect2)) +
  labs(title= "Log(views_youtube) by % of affect words", x = "% of affect words in a talk", y = "log(views_youtube)")
