...
# train and test sets
data_train <- data %>% select(id, duration, views, views_youtube, likes, dislike, tedx, good_bad ) %>% slice(train_index)
data_test  <- data %>% select(id, duration, views, views_youtube, likes, dislike, tedx, good_bad ) %>% slice(-train_index)
...
plot_posemo <- ggplot(talks, aes(posemo, log(views))) + 
  geom_point() +
  geom_smooth(aes(posemo, pred_posemo)) +
  labs(title= "Log(views) by % of positive affect words", x = "% of positive affect words in a talk", y = "log(views)")+
  ylim(8,18)+xlim(0,0.2)
