...
#change duration to usable format
#omit letters
data = data %>% mutate(
...
# train and test sets
data_train <- data %>% select(id, presenter, event, upload_date, duration, views, views_youtube, likes, dislike, tedx, good_bad ) %>% slice(train_index)
data_test  <- data %>% select(id, presenter, event, upload_date, duration, views, views_youtube, likes, dislike, tedx, good_bad ) %>% slice(-train_index)

data = data %>% mutate(
  id = as.factor(id),
  presenter = as.factor(id),
  tedx = as.factor(id)
)
...
grad_rpart <- train(form = good_bad ~ .,
                    data = data_train,
                    method = "rpart",
                    trControl = ctrl_none,
...
# evaluate fitting models -------------------------------------------------

# Save training criterion values
criterion_train <- data_train$good_bad
...
# Evaluate prediction perforamnce -----------------------------------------

# Save criterion values
criterion_test <- data_test$good_bad

# Regression
glm_pred <- predict(grad_glm, newdata = data_test )

# Decision Trees
rpart_pred <- predict(grad_rpart, newdata = data_test )

# Random Forests
rf_pred <- predict(grad_rf, newdata = data_test )
...
lambda_vec <- seq(.000000001, .0003, length=100)


# lasso regression
grad_tune_glm <- train(form = good_bad ~ .-HispanicLatino -Position - State - Department - ManagerName,
                       data = data_train,
                       method = "glmnet",
                       trControl = ctrl_cv,
...
cp_vec <- seq(0, .1, length = 10)

# Decision tree 
grad_tune_rpart <- train(form = good_bad ~ .,
                         data = data_train,
                         method = "rpart",
                         trControl = ctrl_cv,
...
# Random Forest 
grad_tune_rf <- train(form = good_bad ~ .,
                      data = data_train,
                      method = "rf",
                      trControl = ctrl_cv,
...
# Evaluation prediction performance of tuned models -----------------------

glm_tune_pred <- predict(grad_tune_rf, newdata = data_test )

rpart_tune_pred  <- predict(grad_tune_rpart, newdata = data_test )

rf_tune_pred  <- predict(grad_tune_glm, newdata = data_test )
...
model.matrix(good_bad ~ . , data = data)
...
#Selecting all the talks that were filmed from 2000 to 2017
# data <- data %>% filter( upload_date <= "2017-06-01")                           
# data <- data %>% filter( upload_date >= "2017-06-01" & log(views) >= 12)         
...
data %>% select(upload_date, views) %>% arrange(upload_date) %>% ggplot(aes(x = upload_date, y = log(views))) +
  geom_point() + 
